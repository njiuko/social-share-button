window.SocialShareButton =
  ...
  share : (el) ->
    ...
    switch site
      when "email"
        location.href = "mailto:?to=&subject=#{desc}&body=#{url}"
      when "weibo"
      ...
      when "twitter"
        ...
        SocialShareButton.openUrl("https://twitter.com/intent/tweet?url=#{url}&text=#{desc}&hashtags=#{hashtags}#{via_str}", 650, 300)
      when "douban"
      ...
      when "pinterest"
        SocialShareButton.openUrl("http://www.pinterest.com/pin/create/button/?url=#{url}&media=#{img}&description=#{desc}")
      when "linkedin"
